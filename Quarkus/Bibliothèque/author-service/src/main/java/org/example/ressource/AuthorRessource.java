...
public class AuthorRessource {
    ...
    @POST
    public Response save(AuthorDto bookDto){
        return Response.ok(bookService.save(bookDto)).status(201).build();
    }

    @PATCH
    @Path("/{id}")
    public Response update(@PathParam("id")long id, AuthorDto bookDto){
        return Response.ok(bookService.update(id, bookDto)).status(201).build();
    }

    @DELETE
    @Path("/{id}")
    public Response deleteById(@PathParam("id")long id, AuthorDto bookDto){
        return Response.ok(bookService.update(id, bookDto)).status(201).build();
    }
}
